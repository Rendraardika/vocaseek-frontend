...
import { getAdminTalent, getAdminTalents } from "../../../services/admin";
...
function scoreTalentPayload(item) {
  const mapped = mapTalentDetailPayload(item || {});

  return [
    mapped?.birthDate,
    mapped?.birthPlaceAndDate && mapped.birthPlaceAndDate.includes(","),
    mapped?.birthPlace,
    mapped?.gender,
    mapped?.phone,
    mapped?.address,
      ...
      try {
        ...
        if (resolvedTalentId && resolvedTalentId !== String(id) && !hasBirthDateInPrimary) {
          try {
            ...
            // Keep the best available payload when the resolved detail endpoint is unavailable.
          }
        }

        if (
          listMatchedItem &&
          scoreTalentPayload(listMatchedItem) > scoreTalentPayload(matchedItem)
        ) {
          matchedItem = listMatchedItem;
        }
